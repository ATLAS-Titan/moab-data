...
#   bin 2 jobs submitted by non-CSC108 projects, with and without CSC108
#   running active jobs in backfill.
#
#   Bin 2 jobs use between 3750 and 11249 nodes, inclusive.
#
...
def analyze(connection):
  ...
  # WaitTime for bin 2 jobs submitted by projects other than CSC108, when
        ...
        SELECT DISTINCT
            ...
            WHERE
                ...
                AND ((B.ReqNodes IS NULL
                        ...
                        AND (B.ReqProcs / 16) <= 11249)
                    OR (B.ReqNodes IS NOT NULL
                        AND 3750 <= B.ReqNodes
                        AND B.ReqNodes <= 11249))
  ...
  # Create the QQ plot.
    ...
    ax.set(xlabel = "With CSC108 Running", ylabel = "Without CSC108 Running",
        title = "QQ Plot: Wait Times for Bin 2 Jobs")
    ax.grid()
