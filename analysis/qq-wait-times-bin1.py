...
#   bin 1 jobs submitted by non-CSC108 projects, with and without CSC108
#   running active jobs in backfill.
#
#   Bin 1 jobs use 11250 nodes or more.
#
...
def analyze(connection):
  ...
  # WaitTime for bin 1 jobs submitted by projects other than CSC108, when
        ...
        SELECT DISTINCT
            ...
            WHERE
                ...
                AND ((B.ReqNodes IS NULL
                        AND 11250 <= (B.ReqProcs / 16))
                    OR (B.ReqNodes IS NOT NULL
                        AND 11250 <= B.ReqNodes))
  ...
  # Create the QQ plot.
    ...
    ax.set(xlabel = "With CSC108 Running", ylabel = "Without CSC108 Running",
        title = "QQ Plot: Wait Times for Bin 1 Jobs")
    ax.grid()
